...
- **Global Fleet View** — MarineTraffic-style dense global map: ~3,100 synthetic
  ...
  **Click any vessel** to open a detail panel: identity, flag, route
  (origin/destination with real port codes), ATD/ETA, past-track and
  route-forecast mini-maps, navigational status, speed/course/draught,
  **estimated Scope 3 emissions** for that specific vessel, **live weather**
  at its current position (Open-Meteo, real data), and an **AI-generated
  summary** of the vessel's voyage and footprint
- **Singapore Map** — full satellite map (Esri World Imagery) with toggleable
  layers (ports, warehouses, vessels, truck routes)
