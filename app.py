...
page = st.sidebar.radio(
    "Navigate",
    ["Dashboard", "Global Fleet View", "Singapore Map", "Google Maps / Earth View", "Scope 3 Calculator",
...
# ---------------------------------------------------------------------------
    ...
    view_state = pdk.ViewState(latitude=20, longitude=20, zoom=1.3, pitch=0)
    deck = pdk.Deck(
        layers=[layer],
    ...
    event = st.pydeck_chart(
        deck, use_container_width=True, height=560,
    ...
    if selected_vessel_id is not None:
        ...
        if not match.empty:
            v = match.iloc[0]
            st.markdown("---")
            st.markdown(f"### \U0001F6A2 {v['name']}")
            st.caption(f"{v['class']} \u00b7 Flag: {v['flag']} \u00b7 MMSI {v['vessel_id']} \u00b7 Lane: {v['lane']}")

            route_col1, route_col2, route_col3 = st.columns([2, 2, 3])
            route_col1.markdown(f"**{v['origin_country']} {v['origin_code']}**  \n{v['origin_name']}")
            route_col2.markdown(f"**{v['dest_country']} {v['dest_code']}**  \n{v['dest_name']}")
            with route_col3:
                st.progress(float(v["progress"]), text=f"{int(v['progress']*100)}% of voyage complete")
            st.caption(f"ATD: {v['atd'].strftime('%Y-%m-%d %H:%M')}  \u00b7  Reported ETA: {v['eta'].strftime('%Y-%m-%d %H:%M')}")
