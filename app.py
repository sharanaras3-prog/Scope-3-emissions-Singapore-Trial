...
if page == "Dashboard":
    ...
    st.caption("Marine traffic (vessels, ports) and road traffic (trucks, warehouse-to-terminal lanes) combined")
    ...
    # Warehouses (road traffic origin points)
    for _, w in WAREHOUSES.iterrows():
        folium.CircleMarker([w["lat"], w["lon"]], radius=4, color="#a78bfa", fill=True, fill_opacity=0.85,
                             popup=f"{w['name']} (warehouse)").add_to(m)

    # Marine traffic: vessels
    for _, v in vessels_df.iterrows():
        ...
        folium.CircleMarker([lat, lon], radius=5, color="#38bdf8", fill=True, fill_opacity=0.95,
                             popup=f"<b>{v['name']}</b> (vessel)<br>{v['type']} \u00b7 {v['fuel']}<br>"
                                   f"To: {v['dest_name']} \u00b7 {v['speed_knots']} kn").add_to(m)

    # Road traffic: trucks + their warehouse-to-terminal lanes
    for _, t in trucks_df.iterrows():
        folium.PolyLine([[t["from_lat"], t["from_lon"]], [t["to_lat"], t["to_lon"]]],
                         color="#fbbf24", weight=2, opacity=0.7, dash_array="4").add_to(m)
        t_lat = t["from_lat"] + (t["to_lat"] - t["from_lat"]) * t["progress"]
        t_lon = t["from_lon"] + (t["to_lon"] - t["from_lon"]) * t["progress"]
        folium.CircleMarker([t_lat, t_lon], radius=5, color="#f97316", fill=True, fill_opacity=0.95,
                             popup=f"<b>{t['truck_id']}</b> (truck)<br>{t['driver']} \u00b7 {t['fuel']}<br>"
                                   f"{t['from_name']} \u2192 {t['to_name']} \u00b7 Status: {t['status']}").add_to(m)

    st_folium(m, height=460, use_container_width=True, key="dash_map")
    st.markdown(
        '<div style="display:flex;gap:16px;flex-wrap:wrap;font-size:12px;color:#94a3b8;margin-top:-8px;">'
        '<span>\U0001F535 Vessels (marine)</span>'
        '<span>\U0001F7E0 Trucks (road)</span>'
        '<span>\U0001F7E1 Truck routes</span>'
        '<span>\U0001F7E3 Warehouses</span>'
        '<span>\U0001F535 Ports / terminals</span>'
        '</div>',
        unsafe_allow_html=True,
    )
