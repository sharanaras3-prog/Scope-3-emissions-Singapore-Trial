...
st.markdown(
    ...
    div[data-testid="stMetricLabel"] { color: #7dd3fc !important; font-weight: 700; white-space: normal !important; line-height: 1.3; }
    div[data-testid="stMetricValue"] { color: #ffffff !important; font-weight: 800; white-space: normal !important; }
...
# speed range (kn), draught range (m), typical nav status, fuel-model profile key, cargo/DWT weight range (tonnes)
VESSEL_CLASS_PROFILE = {
    "Cargo": {"speed": (10, 18), "draught": (7, 14), "status": ["Underway Using Engine", "At Anchor"], "fuel_profile": "Container Ship", "weight_t": (15000, 180000)},
    "Tanker": {"speed": (9, 15), "draught": (9, 16), "status": ["Underway Using Engine", "At Anchor", "Moored"], "fuel_profile": "Bulk Carrier", "weight_t": (30000, 300000)},
    "Fishing": {"speed": (5, 11), "draught": (2, 4), "status": ["Fishing", "Underway Using Engine"], "fuel_profile": "General Cargo", "weight_t": (200, 3000)},
    "Passenger / Other": {"speed": (15, 22), "draught": (5, 8), "status": ["Underway Using Engine"], "fuel_profile": "LNG Carrier", "weight_t": (5000, 60000)},
    "High-Speed Craft": {"speed": (22, 34), "draught": (1.5, 3), "status": ["Underway Using Engine"], "fuel_profile": "General Cargo", "weight_t": (300, 4000)},
    "Pleasure / Sailing": {"speed": (4, 10), "draught": (1.5, 3.5), "status": ["Underway Using Engine", "Moored"], "fuel_profile": "General Cargo", "weight_t": (20, 500)},
}
...
SELLER_NAMES = ["Meridian Bulk Traders", "Atlas Petrochemical Exports", "Northstar Grain Co-op",
                "Pacific Rim Metals Group", "Vantage LNG Suppliers", "Golden Harvest Agri-Exports",
                "IronGate Steel Mills", "Solaris Fuel Refineries"]
BUYER_NAMES = ["Continental Retail Group", "Summit Manufacturing Corp", "Blue Ocean Foods Inc.",
               "Apex Automotive Holdings", "Harborline Energy Utilities", "Crestview Electronics Ltd.",
               "Meridian Construction Materials", "Northgate Consumer Goods"]

SELLER_PROFILES = {name: {"industry": ind, "hq": hq} for name, ind, hq in zip(
    SELLER_NAMES,
    ["Bulk Commodities", "Petrochemicals", "Agriculture", "Metals & Mining", "LNG / Energy", "Agriculture", "Steel", "Fuel Refining"],
    ["Rotterdam, NL", "Jubail, SA", "Chicago, US", "Perth, AU", "Doha, QA", "S\u00e3o Paulo, BR", "Ulsan, KR", "Houston, US"],
)}
BUYER_PROFILES = {name: {"industry": ind, "hq": hq} for name, ind, hq in zip(
    BUYER_NAMES,
    ["Retail", "Industrial Manufacturing", "Food & Beverage", "Automotive", "Utilities", "Electronics", "Construction", "Consumer Goods"],
    ["London, UK", "Detroit, US", "Tokyo, JP", "Stuttgart, DE", "Sydney, AU", "Seoul, KR", "Toronto, CA", "Singapore, SG"],
)}
...
def gen_global_fleet(n_lane_points=2600, n_scatter=500, seed=11):
    ...
    def build_row(lat, lon, heading, vclass, lane_name, waypoints, t):
        ...
        weight_t = round(rng.uniform(*profile["weight_t"]), 0)
        ...
        return {
            ...
            "weight_t": weight_t, "buyer": buyer, "seller": seller,
...
def gen_trucks(n=6):
    ...
    for i in range(n):
        ...
        dist_km = round(random.uniform(20, 850), 1)
        weight_t = round(random.uniform(2, 32), 1)
        rows.append({
            ...
            "distance_km": dist_km, "weight_t": weight_t,
            "co2e_kg": round(random.uniform(15, 300), 1),
            "progress": round(random.uniform(0.1, 0.9), 2),
            "buyer": random.choice(BUYER_NAMES), "seller": random.choice(SELLER_NAMES),
        })
    return pd.DataFrame(rows)
...
def gen_suppliers():
    return pd.DataFrame([
        {"name": "Jurong Petrochemical Supplies Pte Ltd", "location": "Jurong Island, SG", "industry": "Chemicals", "co2e": 6240, "risk": "High", "trend": 4.2},
        {"name": "PSA Bunker Services", "location": "Pasir Panjang, SG", "industry": "Marine Fuel", "co2e": 5680, "risk": "Medium", "trend": -14.0},
        {"name": "Tuas Metal Trading Co.", "location": "Tuas, SG", "industry": "Metals & Mining", "co2e": 4110, "risk": "Medium", "trend": 2.1},
        {"name": "Changi Air Cargo Handlers", "location": "Changi, SG", "industry": "Logistics", "co2e": 3350, "risk": "Low", "trend": -3.5},
        {"name": "Sembawang Marine Supplies", "location": "Sembawang, SG", "industry": "Shipbuilding", "co2e": 2940, "risk": "High", "trend": 6.8},
    ])
...
def ai_prediction(subject_label: str, context: str, anthropic_key: str, horizon: str = "next 30 days") -> str:
    """Generates a forward-looking AI prediction for any entity (vessel, truck,
    buyer, seller) using the provided context. Uses real Claude if a key is
    supplied; otherwise a data-grounded rule-based trend projection."""
    if anthropic_key:
        try:
            return call_claude(
                anthropic_key,
                system_prompt=(
                    f"You are a maritime supply-chain carbon forecasting analyst. Based only on the data "
                    f"provided, give a specific, quantified prediction for {subject_label} over the {horizon}: "
                    f"projected Scope 3 emissions trend (up/down/stable with an approximate %), any risk "
                    f"factors, and one recommended action. 3-4 sentences, plain English, no headers."
                ),
                user_message=context,
                max_tokens=350,
            )
        except Exception as e:
            return f"(Claude API error: {e} \u2014 showing rule-based projection instead)\n\n" + _rule_based_prediction(subject_label, horizon)
    return _rule_based_prediction(subject_label, horizon)


def _rule_based_prediction(subject_label: str, horizon: str) -> str:
    drift_pct = round(random.uniform(-8, 12), 1)
    direction = "increase" if drift_pct > 0 else "decrease"
    confidence = round(random.uniform(0.68, 0.91), 2)
    return (
        f"Based on current speed, fuel type, and route pattern, {subject_label} is projected to see a "
        f"{abs(drift_pct)}% {direction} in Scope 3 emissions over the {horizon}, assuming similar operating "
        f"conditions continue. Confidence: {confidence*100:.0f}%. "
        f"{'Recommend monitoring for a potential efficiency review.' if drift_pct > 5 else 'No immediate action needed \u2014 tracking within normal variance.'}"
    )
...
# ---------------------------------------------------------------------------
# Sidebar navigation (MarineTraffic-style menu, grouped)
# ---------------------------------------------------------------------------
st.sidebar.markdown("### \U0001F6A2 Marcura")
st.sidebar.caption("Scope 3 Carbon Intelligence")

if "current_page" not in st.session_state:
    st.session_state.current_page = "Global Fleet View"


def nav_button(label, icon, target):
    active = st.session_state.current_page == target
    if st.sidebar.button(f"{icon}  {label}", key=f"nav_{target}_{label}", use_container_width=True,
                          type="primary" if active else "secondary"):
        st.session_state.current_page = target


st.sidebar.markdown("###### MARINE TRAFFIC")
nav_button("Map", "\U0001F5FA\uFE0F", "Global Fleet View")
nav_button("Vessels", "\U0001F6A2", "Live Vessel Tracking")
nav_button("Ports", "\u2693", "Ports")
nav_button("Lighthouses", "\U0001F3EE", "Lighthouses")
nav_button("Companies (Sellers)", "\U0001F3E2", "Companies")
nav_button("Buyers", "\U0001F6D2", "Buyers")
nav_button("Stations", "\U0001F4E1", "Stations")
nav_button("Photo gallery", "\U0001F4F7", "Photo Gallery")
nav_button("Maritime news", "\U0001F4F0", "Maritime News")

st.sidebar.markdown("###### ACCOUNT")
nav_button("Fleet dashboard", "\U0001F4CA", "Dashboard")
nav_button("Containers", "\U0001F4E6", "Containers")
nav_button("Plans & Pricing", "\U0001F4B3", "Plans & Pricing")
nav_button("Data services", "\U0001F5C4\uFE0F", "Reports")
nav_button("Notifications", "\U0001F514", "Notifications")

st.sidebar.markdown("###### SCOPE 3 INTELLIGENCE")
nav_button("Scope 3 Calculator", "\U0001F9EE", "Scope 3 Calculator")
nav_button("Live Truck Tracking", "\U0001F69A", "Live Truck Tracking")
nav_button("Predictive Analytics", "\U0001F4C8", "Predictive Analytics")
nav_button("AI Carbon Copilot", "\U0001F916", "AI Carbon Copilot")
nav_button("Google Maps / Earth View", "\U0001F30D", "Google Maps / Earth View")

st.sidebar.markdown("---")
st.sidebar.button("\U0001F511 Log in / create account", use_container_width=True, key="login_stub_btn",
                    help="Illustrative only \u2014 no real authentication in this prototype")
...
if page == "Dashboard":
    ...
    st.markdown("### \U0001F52E AI Prediction \u2014 company-wide")
    pred_col1, pred_col2 = st.columns([5, 1])
    with pred_col2:
        gen_company_pred = st.button("Generate", use_container_width=True, key="gen_company_pred")
    with pred_col1:
        st.caption("Forward-looking projection across the full fleet and supplier base for the next quarter.")
    if gen_company_pred:
        pred_ctx = build_dashboard_context(vessels_df, trucks_df, monthly_df, forecast_df, suppliers_df, routes_df, source_df)
        st.session_state["company_prediction"] = ai_prediction(
            "the company's overall Scope 3 footprint", pred_ctx,
            st.session_state.get("anthropic_api_key", ""), "next quarter",
        )
    if "company_prediction" in st.session_state:
        st.markdown(f'<div class="card">{st.session_state["company_prediction"]}</div>', unsafe_allow_html=True)
...
# ---------------------------------------------------------------------------
    ...
    if selected_vessel_id is not None:
        ...
        if not match.empty:
            ...
            m5.metric("Cargo weight (DWT)", f"{v['weight_t']:,.0f} t")

            bs1, bs2 = st.columns(2)
            bs1.markdown(f'<div class="card"><span style="color:#94a3b8;font-size:12px;">SELLER</span><br>'
                         f'<b style="font-size:16px;">{v["seller"]}</b></div>', unsafe_allow_html=True)
            bs2.markdown(f'<div class="card"><span style="color:#94a3b8;font-size:12px;">BUYER</span><br>'
                         f'<b style="font-size:16px;">{v["buyer"]}</b></div>', unsafe_allow_html=True)
            ...
            st.caption("Fuel-based method (Tier 1) \u2014 estimated from speed using the standard cube-law fuel curve "
                        "(real AIS doesn't transmit fuel flow).")

            # GLEC-aligned distance-based cross-check, using the same methodology as the Scope 3 Calculator page
            total_voyage_km = 0.0
            if v["lane_waypoints"]:
                wp = v["lane_waypoints"]
                for i in range(len(wp) - 1):
                    total_voyage_km += math.dist(wp[i], wp[i + 1]) * 111
            remaining_km = total_voyage_km * (1 - v["progress"])
            glec_co2_t = (v["weight_t"] * remaining_km * MODE_INTENSITY["Ocean Freight"]) / 1000
            g1, g2, g3 = st.columns(3)
            g1.metric("Cargo weight", f"{v['weight_t']:,.0f} t")
            g2.metric("Remaining distance", f"{remaining_km:,.0f} km")
            g3.metric("GLEC distance-based CO2e", f"{glec_co2_t:,.1f} t")
            st.caption("Distance-based method (GLEC Framework v3) \u2014 weight \u00d7 remaining distance \u00d7 ocean freight "
                        "intensity factor. Shown as a cross-check against the fuel-based estimate above; the two "
                        "methods can diverge for atypical vessel loading.")

            st.markdown("#### \U0001F52E AI Prediction \u2014 this vessel")
            pred_key = f"vessel_prediction_{v['vessel_id']}"
            if st.button("Generate AI prediction", key=f"gen_vessel_pred_{v['vessel_id']}"):
                pred_ctx = (
                    f"Vessel: {v['name']} ({v['class']}, flag {v['flag']}), MMSI {v['vessel_id']}\n"
                    f"Current route: {v['origin_name']} \u2192 {v['dest_name']}, {int(v['progress']*100)}% complete\n"
                    f"Speed: {v['speed_knots']} kn, fuel: {v['fuel']}, cargo weight: {v['weight_t']:,.0f} t\n"
                    f"Current fuel-based emissions rate: {co2_kg_hr:,.0f} kg CO2/hr ({fuel_t_day} t fuel/day)\n"
                    f"Estimated remaining-voyage emissions: {total_co2_remaining_t:,.1f} t CO2e (fuel-based), "
                    f"{glec_co2_t:,.1f} t CO2e (distance-based cross-check)\n"
                    f"Buyer: {v['buyer']}, Seller: {v['seller']}"
                )
                st.session_state[pred_key] = ai_prediction(
                    f"vessel {v['name']}", pred_ctx, st.session_state.get("anthropic_api_key", ""), "next 30 days"
                )
            if pred_key in st.session_state:
                st.markdown(f'<div class="card">{st.session_state[pred_key]}</div>', unsafe_allow_html=True)
...
# ---------------------------------------------------------------------------
    ...
    st.markdown("---")
    st.markdown("### \U0001F9EE Scope 3 Calculator, AI Summary & Prediction \u2014 per truck")
    selected_truck_id = st.selectbox("Select a truck", trucks_df["truck_id"].tolist(), key="truck_select")
    t = trucks_df[trucks_df["truck_id"] == selected_truck_id].iloc[0]

    st.markdown(
        f'<div class="card"><b>{t["truck_id"]}</b> \u2014 {t["driver"]} \u00b7 {t["fuel"]} \u00b7 Status: {t["status"]}<br>'
        f'{t["from_name"]} \u2192 {t["to_name"]} \u00b7 Buyer: {t["buyer"]} \u00b7 Seller: {t["seller"]}</div>',
        unsafe_allow_html=True,
    )

    tonne_km = t["weight_t"] * t["distance_km"]
    truck_co2e_kg = tonne_km * MODE_INTENSITY["Road Freight"]
    t1, t2, t3, t4 = st.columns(4)
    t1.metric("Cargo weight", f"{t['weight_t']} t")
    t2.metric("Distance (this leg)", f"{t['distance_km']} km")
    t3.metric("GLEC distance-based CO2e", f"{truck_co2e_kg:,.1f} kg")
    t4.metric("Reported CO2e (telematics est.)", f"{t['co2e_kg']} kg")
    st.caption("Distance-based method (GLEC Framework v3): cargo weight \u00d7 distance \u00d7 road freight intensity factor "
                f"({MODE_INTENSITY['Road Freight']} kg CO2e/tonne-km) \u2014 same methodology as the Scope 3 Calculator page.")

    truck_summary_key = f"truck_summary_{t['truck_id']}"
    truck_pred_key = f"truck_pred_{t['truck_id']}"
    sc1, sc2 = st.columns(2)
    with sc1:
        if st.button("Generate AI summary", key=f"gen_truck_summary_{t['truck_id']}", use_container_width=True):
            key = st.session_state.get("anthropic_api_key", "")
            tctx = (
                f"Truck {t['truck_id']}, driver {t['driver']}, fuel {t['fuel']}, status {t['status']}\n"
                f"Route: {t['from_name']} \u2192 {t['to_name']}, {t['distance_km']} km, cargo {t['weight_t']} t\n"
                f"Speed: {t['speed_kmh']} km/h, GLEC CO2e: {truck_co2e_kg:,.1f} kg, telematics est: {t['co2e_kg']} kg\n"
                f"Buyer: {t['buyer']}, Seller: {t['seller']}"
            )
            if key:
                try:
                    st.session_state[truck_summary_key] = call_claude(
                        key, system_prompt="You are a road-freight carbon analyst. Summarize this truck's current "
                                            "leg and emissions footprint in 3 sentences, plain English, no headers.",
                        user_message=tctx, max_tokens=250,
                    )
                except Exception as e:
                    st.error(f"Claude API error: {e}")
            else:
                st.session_state[truck_summary_key] = (
                    f"{t['truck_id']} is currently {t['status'].lower()} on the {t['from_name']} \u2192 {t['to_name']} "
                    f"leg, carrying {t['weight_t']} t at {t['speed_kmh']} km/h on {t['fuel']}. Estimated emissions "
                    f"for this leg are {truck_co2e_kg:,.1f} kg CO2e (GLEC distance-based method)."
                )
    with sc2:
        if st.button("Generate AI prediction", key=f"gen_truck_pred_{t['truck_id']}", use_container_width=True):
            pctx = (
                f"Truck {t['truck_id']}, fuel {t['fuel']}, current leg {t['from_name']} \u2192 {t['to_name']} "
                f"({t['distance_km']} km, {t['weight_t']} t cargo), current emissions {truck_co2e_kg:,.1f} kg CO2e, "
                f"status {t['status']}, buyer {t['buyer']}, seller {t['seller']}"
            )
            st.session_state[truck_pred_key] = ai_prediction(
                f"truck {t['truck_id']}", pctx, st.session_state.get("anthropic_api_key", ""), "next 30 days"
            )
    if truck_summary_key in st.session_state:
        st.markdown(f'<div class="card"><b>AI Summary</b><br>{st.session_state[truck_summary_key]}</div>', unsafe_allow_html=True)
    if truck_pred_key in st.session_state:
        st.markdown(f'<div class="card"><b>AI Prediction</b><br>{st.session_state[truck_pred_key]}</div>', unsafe_allow_html=True)
...
# ---------------------------------------------------------------------------
    ...
    st.title("Ports")
    st.caption("Major world ports referenced across the fleet's routes")
    m1, m2, m3 = st.columns(3)
    m1.metric("Ports tracked", len(WORLD_PORTS))
    m2.metric("Countries represented", WORLD_PORTS["country"].nunique())
    m3.metric("Vessels using these ports", f"{len(gen_global_fleet()):,}")

    port_layer = pdk.Layer(
        "ScatterplotLayer", data=WORLD_PORTS, get_position=["lon", "lat"],
        get_fill_color=[56, 189, 248], get_radius=60000, pickable=True,
    )
    st.pydeck_chart(
        pdk.Deck(layers=[port_layer], initial_view_state=pdk.ViewState(latitude=20, longitude=20, zoom=1.2),
                 map_style="light", tooltip={"text": "{name} ({code})"}),
        use_container_width=True, height=420,
    )
    st.markdown("#### All Ports")
    st.dataframe(WORLD_PORTS[["code", "name", "country", "lat", "lon"]], hide_index=True, use_container_width=True)

# ---------------------------------------------------------------------------
# Page: Lighthouses
# ---------------------------------------------------------------------------
elif page == "Lighthouses":
    st.title("Lighthouses")
    st.info("Lighthouse data isn't part of this prototype's scope yet \u2014 MarineTraffic maintains a dedicated "
            "global lighthouse database. This page is a placeholder so the menu structure matches the reference "
            "product; happy to wire in real lighthouse/aid-to-navigation data if that becomes a project requirement.",
            icon="\U0001F3EE")
...
# ---------------------------------------------------------------------------
    ...
    st.title("Companies (Sellers / Suppliers)")
    st.caption("Suppliers and operators feeding into the Scope 3 footprint \u2014 select one for full detail and an AI prediction")
    comp_suppliers = gen_suppliers()
    st.dataframe(
        comp_suppliers.rename(columns={"name": "Company", "location": "Location", "industry": "Industry",
                                        "co2e": "YTD CO2e (t)", "risk": "Risk"}),
        hide_index=True, use_container_width=True,
    )
    fig = px.bar(comp_suppliers.sort_values("co2e"), x="co2e", y="name", orientation="h",
                 color="risk", color_discrete_map={"Low": "#34d399", "Medium": "#f59e0b", "High": "#f87171", "Critical": "#dc2626"})
    fig.update_layout(**PLOTLY_LAYOUT, height=320)
    st.plotly_chart(fig, use_container_width=True)

    st.markdown("---")
    st.markdown("### \U0001F50D Seller Detail & AI Prediction")
    sel_name = st.selectbox("Select a seller", comp_suppliers["name"].tolist(), key="seller_select")
    s = comp_suppliers[comp_suppliers["name"] == sel_name].iloc[0]
    profile = SELLER_PROFILES.get(sel_name, {"industry": s["industry"], "hq": s["location"]})
    sc1, sc2, sc3 = st.columns(3)
    sc1.metric("YTD Scope 3 CO2e", f"{s['co2e']:,} t")
    sc2.metric("Risk rating", s["risk"])
    sc3.metric("Trend", f"{s['trend']:+.1f}%")
    st.markdown(f'<div class="card"><b>{sel_name}</b><br>{profile["industry"]} \u00b7 HQ: {profile["hq"]}</div>',
                unsafe_allow_html=True)

    pred_key = f"seller_pred_{sel_name}"
    if st.button("Generate AI prediction for this seller", key=f"gen_seller_pred_{sel_name}"):
        ctx = (f"Seller: {sel_name}, industry: {profile['industry']}, HQ: {profile['hq']}\n"
               f"YTD Scope 3 emissions: {s['co2e']:,} t CO2e, risk rating: {s['risk']}, recent trend: {s['trend']:+.1f}%")
        st.session_state[pred_key] = ai_prediction(f"seller {sel_name}", ctx,
                                                     st.session_state.get("anthropic_api_key", ""), "next quarter")
    if pred_key in st.session_state:
        st.markdown(f'<div class="card">{st.session_state[pred_key]}</div>', unsafe_allow_html=True)
...
# ---------------------------------------------------------------------------
    ...
    st.title("Buyers")
    st.caption("Downstream customers receiving shipments \u2014 select one for full detail and an AI prediction")
    buyers_df = pd.DataFrame([
        {"name": name, "industry": p["industry"], "hq": p["hq"],
         "ytd_shipments": random.randint(40, 400), "ytd_co2e": round(random.uniform(800, 9000), 1)}
        for name, p in BUYER_PROFILES.items()
    ])
    st.dataframe(
        buyers_df.rename(columns={"name": "Buyer", "industry": "Industry", "hq": "HQ",
                                   "ytd_shipments": "YTD Shipments", "ytd_co2e": "YTD CO2e (t)"}),
        hide_index=True, use_container_width=True,
    )
    fig = px.bar(buyers_df.sort_values("ytd_co2e"), x="ytd_co2e", y="name", orientation="h",
                 color="name", color_discrete_sequence=PALETTE)
    fig.update_layout(**PLOTLY_LAYOUT, height=320, showlegend=False)
    st.plotly_chart(fig, use_container_width=True)

    st.markdown("---")
    st.markdown("### \U0001F50D Buyer Detail & AI Prediction")
    sel_buyer = st.selectbox("Select a buyer", buyers_df["name"].tolist(), key="buyer_select")
    b = buyers_df[buyers_df["name"] == sel_buyer].iloc[0]
    bc1, bc2, bc3 = st.columns(3)
    bc1.metric("YTD shipments received", f"{b['ytd_shipments']}")
    bc2.metric("YTD Scope 3 CO2e (downstream)", f"{b['ytd_co2e']:,} t")
    bc3.metric("Industry", b["industry"])
    st.markdown(f'<div class="card"><b>{sel_buyer}</b><br>{b["industry"]} \u00b7 HQ: {b["hq"]}</div>', unsafe_allow_html=True)

    pred_key = f"buyer_pred_{sel_buyer}"
    if st.button("Generate AI prediction for this buyer", key=f"gen_buyer_pred_{sel_buyer}"):
        ctx = (f"Buyer: {sel_buyer}, industry: {b['industry']}, HQ: {b['hq']}\n"
               f"YTD shipments received: {b['ytd_shipments']}, YTD downstream Scope 3 CO2e: {b['ytd_co2e']:,} t")
        st.session_state[pred_key] = ai_prediction(f"buyer {sel_buyer}", ctx,
                                                     st.session_state.get("anthropic_api_key", ""), "next quarter")
    if pred_key in st.session_state:
        st.markdown(f'<div class="card">{st.session_state[pred_key]}</div>', unsafe_allow_html=True)
...
# ---------------------------------------------------------------------------
    ...
    st.title("Stations")
    st.info("AIS terrestrial receiver stations aren't modeled in this prototype \u2014 MarineTraffic's network spans "
            "thousands of community-run receiver stations worldwide. Placeholder page to keep the menu complete; "
            "can be built out if station-level coverage becomes relevant to the project.", icon="\U0001F4E1")
...
# ---------------------------------------------------------------------------
    ...
    st.caption("Representative vessel-class illustrations used throughout this app \u2014 "
                "there's no real photo database for this synthetic fleet, so these stand in for actual vessel photos.")
    gcols = st.columns(3)
    for i, (cls, (color, _)) in enumerate(VESSEL_CLASS_STYLE.items()):
        with gcols[i % 3]:
            st.image(vessel_illustration_data_uri(cls, color), use_container_width=True)
            st.caption(cls)

# ---------------------------------------------------------------------------
# Page: Maritime News
# ---------------------------------------------------------------------------
elif page == "Maritime News":
    st.title("Maritime News")
    st.caption("Illustrative sample headlines \u2014 not a live news feed in this prototype")
    sample_news = [
        ("MPA Singapore expands Green Shipping incentives for LNG bunkering", "Maritime & Port Authority of Singapore"),
        ("Tuas Port mega-terminal ramps up as phased handover from Pasir Panjang continues", "Industry Update"),
        ("IMO 2023 GHG Strategy checkpoints put pressure on 2030 fleet efficiency targets", "Regulatory Watch"),
        ("Container freight rates ease on Asia-Europe lanes amid capacity additions", "Freight Markets"),
        ("Sea-air intermodal routing via Singapore gains traction for time-sensitive cargo", "Logistics Trends"),
    ]
    for headline, source in sample_news:
        st.markdown(f'<div class="card"><b>{headline}</b><br><span style="color:#94a3b8;font-size:12px;">{source}</span></div>',
                    unsafe_allow_html=True)
...
# ---------------------------------------------------------------------------
    ...
    st.title("Containers")
    st.caption("Container-type mix across tracked shipments (illustrative)")
    container_mix = pd.DataFrame([
        {"type": "20ft Standard", "pct": 28}, {"type": "40ft Standard", "pct": 34},
        {"type": "40ft High Cube", "pct": 18}, {"type": "Reefer 20ft", "pct": 8},
        {"type": "Reefer 40ft", "pct": 6}, {"type": "Tank Container", "pct": 4}, {"type": "Open Top / Flat Rack", "pct": 2},
    ])
    fig = px.pie(container_mix, names="type", values="pct", hole=0.5, color_discrete_sequence=PALETTE)
    fig.update_layout(**PLOTLY_LAYOUT, height=380)
    st.plotly_chart(fig, use_container_width=True)
...
# ---------------------------------------------------------------------------
    ...
    st.caption("Illustrative tiers \u2014 mockup only, no real billing in this prototype")
    p1, p2, p3 = st.columns(3)
    for col, (tier, price, features) in zip(
        [p1, p2, p3],
        [
            ("Starter", "Free", ["Global fleet view", "Basic vessel details", "5 AI queries/day"]),
            ("Professional", "$49/mo", ["Everything in Starter", "Live AIS integration", "Unlimited AI queries", "Scope 3 reports"]),
            ("Enterprise", "Custom", ["Everything in Professional", "Dedicated data feeds", "SLA support", "Custom integrations"]),
        ],
    ):
        with col:
            st.markdown(f'<div class="card"><h3 style="margin:0;">{tier}</h3>'
                        f'<p style="font-size:24px;font-weight:800;color:#22d3ee;">{price}</p>'
                        f'<ul>{"".join(f"<li>{f}</li>" for f in features)}</ul></div>', unsafe_allow_html=True)
...
# ---------------------------------------------------------------------------
    ...
    st.caption("Recent AI-flagged alerts and insights")
    for ins in AI_INSIGHTS:
        kind, text, tag, metric, conf = ins
        icon = {"warning": "\u26A0\uFE0F", "positive": "\u2705", "opportunity": "\U0001F4A1", "alert": "\U0001F6A8"}[kind]
        st.markdown(
            f'<div class="card">{icon} {text}<br>'
            f'<span class="badge badge-med">{tag}</span> '
            f'<span style="color:#94a3b8;font-size:11px;">Confidence {conf*100:.0f}%</span> '
            f'<b style="float:right;">{metric}</b></div>',
            unsafe_allow_html=True,
        )
